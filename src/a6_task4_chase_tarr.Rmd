...
ml <- ggplot(data = mono_lake_2, aes(x = Year, y = lake_level_ft)) +
  ...
  labs(title = "Figure 1: Mono Lake Level Per Year (ft above sea level)", 
       x = "Year",
       y = "Level (ft)",
       caption = "Source: https://www.monobasinresearch.org/") +
...
**Figure 1** represents data points of the recorded feet above sea level for Mono Lake each year.The *highest recorded value* above sea level is represented by a *green dot*. This value is then traced with a red line to the point at which a *land bridge emerges* represented by the *red dot in 1977*. This land bridge connects the mainland to the islands which allows predators to access major bird nesting sites negatively affecting migratory bird species and disrupting local ecosystems. The decrease from 1918 to 1977 can be attributed to human interaction in which the city of Los Angeles re-routed a portion of the water for urban purposes that fed the Mono County Basin prior. This, compounded with recent drought, has caused the decline seen in the last century. The initial decline just after 1918 was due to the initial "draining" as some call it in which Los Angeles first began funneling water into the city. The lake was first recorded at 6427 feet above sea level in 1850, and currently sits near 6381 above sea level which was the last recording in 2017, but has recently reached as low as 6372.30 feet above sea level in the 80's. A *small increase* just before 1940 was quickly negated by a sharp decline from 1941 to 1982 due to the Los Angeles department of Water and Power extending the Los Angeles Aqueduct System more north (shown by the *orange dot*) into the Mono Basin therefore pulling more water that would otherwise go into the lake.
